...
从测试集中选取了 3 个典型时间窗口（每个约 50 个时间点），展示模型的预测效果：
...
- 蓝色实线：真实值
- 红色虚线：预测值
- 每行展示一个时间窗口（约 50 个连续时间点），分别来自测试集的开始、中间和结束时段

**如何理解这个图？**
- 横坐标是 **连续的时间轴**（约 4 小时的数据）
- 每个时间点都有一个预测值（例如 t+1 步预测就是预测 1 步后的浊度）
- 折线对比的是：在这段时间内，每个时刻的预测值与真实值的差异
- 左右两列分别展示 t+1 步（短期）和 t+6 步（长期）的预测效果
