...
class PSOOptimizer(BaseSolver):
    """粒子群优化 (Particle Swarm Optimization)"""
    
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        optimizer_config = config.get('optimizer', {})
        pso_hyperparams = config.get('optimizer_hyperparams', {}).get('pso', {})
        constraints = config.get('constraints', {})
        
        self.n_particles = optimizer_config.get('population_size', 50)
        self.max_iter = optimizer_config.get('max_iterations', 100)
        self.w = pso_hyperparams.get('w', 0.7)      # 惯性权重
        self.c1 = pso_hyperparams.get('c1', 1.5)    # 个体学习因子
        self.c2 = pso_hyperparams.get('c2', 1.5)    # 社会学习因子
        self.rate_limit = constraints.get('dose_rate_limit', 5.0)
...
class SAOptimizer(BaseSolver):
    """模拟退火 (Simulated Annealing)"""
    
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        optimizer_config = config.get('optimizer', {})
        sa_hyperparams = config.get('optimizer_hyperparams', {}).get('sa', {})
        constraints = config.get('constraints', {})
        
        self.temp_init = sa_hyperparams.get('temp_init', 100.0)
        self.temp_min = sa_hyperparams.get('temp_min', 1e-3)
        self.alpha = sa_hyperparams.get('alpha', 0.95)
        self.max_iter = optimizer_config.get('max_iterations', 100)
        self.rate_limit = constraints.get('dose_rate_limit', 5.0)
...
class DEOptimizer(BaseSolver):
    """差分进化 (Differential Evolution)"""
    
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        optimizer_config = config.get('optimizer', {})
        de_hyperparams = config.get('optimizer_hyperparams', {}).get('de', {})
        constraints = config.get('constraints', {})
        
        self.n_population = optimizer_config.get('population_size', 50)
        self.max_iter = optimizer_config.get('max_iterations', 100)
        self.F = de_hyperparams.get('f', 0.8)       # 缩放因子
        self.CR = de_hyperparams.get('cr', 0.9)     # 交叉概率
        self.rate_limit = constraints.get('dose_rate_limit', 5.0)
...
def create_optimizer(config: Dict[str, Any]) -> BaseSolver:
    """
    根据配置创建优化求解器
    ...
    optimizer_type = config.get('optimizer', {}).get('type', 'pso')
