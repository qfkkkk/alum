...
from best_pump_scheduler import main, start_scheduler, stop_scheduler
...
def run_api_only():
    """仅运行API服务（不启动定时任务）"""
    import threading
    from best_pump_scheduler import app
    
    logger.info("启动模式：仅API服务（无定时任务）")
    logger.info("API端点：http://0.0.0.0:5001/pump_station/next_day_pump")
...
def run_scheduler_only():
    ...
    logger.info("定时任务：每天0点执行24小时泵组调度")
...
def run_full_service():
    ...
    from best_pump_scheduler import main as scheduler_main
    scheduler_main()
...
def show_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  最新调度结果: {'有' if status['has_latest_schedule'] else '无'}")
            if status['has_latest_schedule']:
                print(f"  最新调度时间: {status['schedule_time']}")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='最佳水泵24小时调度服务管理')
